...
enum VType {
  NUM_T, BOOL_T, FUN_T, WORD_T, NONE_T
};

class Value {
  private:
    // The value is either an int, a bool, char* or a pointer to a Lambda.
    union {
      int num;
      bool tf;
      Lambda* func;
  ...
  public:
    ...
    Value(const char* s) :type(WORD_T) { val.txt = s; }
    ...
    const char* word() { return val.txt; }
    ...
    bool operator==(const Value& other) {
      switch(type) {
        ...
        case WORD_T: return val.txt == other.val.txt;
